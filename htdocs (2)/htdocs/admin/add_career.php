...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("
            INSERT INTO career_paths (title, description, required_skills, education_path)
            VALUES (?, ?, ?, ?)
        ");
        $stmt->execute([$title, $description, $required_skills, $education_path]);
        
        $_SESSION['flash_success'] = 'Career path added successfully!';
...
<style>
    ...
    /* Soft Pastel Sage Green Submit Button */
    .btn-submit {
...
<div class="edit-layout-container">
    <div class="workspace-inner-board">
        ...
        <h3 class="section-title">💼 Add New Career Path</h3>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <textarea name="description" rows="4" required placeholder="Describe the career path..."></textarea>
            </div>

            <div class="form-group">
                <label>Required Skills (comma-separated)</label>
                <textarea name="required_skills" rows="3" required placeholder="e.g., PHP, JavaScript, SQL"></textarea>
            </div>

            <div class="form-group">
                <label>Education Path</label>
                <textarea name="education_path" rows="2" placeholder="e.g., Bachelor of Computer Science"></textarea>
            </div>
            
            <div class="actions-row">
                <button type="submit" class="btn btn-submit">Add Career Path</button>
                <a href="manage_career.php" class="btn btn-cancel">Cancel</a>
            </div>
        </form>
